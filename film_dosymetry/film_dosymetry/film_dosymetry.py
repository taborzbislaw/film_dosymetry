...
class film_dosymetryWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
    ...
    def monitorAndCompleteCalculations(self,img, STARTS, ENDS, COL_START, COL_END) -> None:     
        ...
        if not self.stop_event.is_set():
            ...
            saveDirName = os.path.dirname(self._parameterNode.inputVolume.GetStorageNode().GetFileName())
            saveFileName = saveDirName + '/CalibratedFilm.nrrd'
            saveImg = sitk.GetImageFromArray(calibrated_image)
            saveImg.SetOrigin(self._parameterNode.inputVolume.GetOrigin())
            saveImg.SetSpacing(self._parameterNode.inputVolume.GetSpacing())
    ...
    def onApplyButton(self) -> None:
        ...
        if os.path.isfile(path):
            ...
            try:
                ...
                BOX = self.ui.boxSize.value
                NORM_FACTOR = self.ui.doubleSpinBoxNormalization.value
                means = [[np.mean(img[p[1]-BOX:p[1]+BOX,p[0]-BOX:p[0]+BOX,channel]/NORM_FACTOR) for channel in [0,1,2]] for p in pointsITK]
                stds =  [[np.std(img[p[1]-BOX:p[1]+BOX,p[0]-BOX:p[0]+BOX,channel]/NORM_FACTOR) for channel in [0,1,2]] for p in pointsITK]

                normalizations = [{'dose':d,'means':m,'stds':s} for d,m,s in zip(doses,means,stds)]
                fname = workDir + '/Resources/normalizations.json'
                f=open(fname,'w')
                print(json.dumps(normalizations, indent=4),file = f)
                f.close()
            except:
                pass
            ...
            for n in range(self.ui.spinBoxNumberOfWorkers.value):
                try:
                    ...
                    process = subprocess.Popen([slicerDir + "/bin/PythonSlicer", workDir + '/optimize.py',str(n),
                              str(STARTS[n]),str(ENDS[n]),str(COL_START),str(COL_END),str(self.ui.doubleSpinBoxTolerance.value),str(self.ui.spinBoxIterations.value),
                              str(self.ui.doubleSpinBoxNormalization.value),str(self.ui.doubleSpinBoxMaxDose.value)], stdout=subprocess.DEVNULL,    
            ...
            self.monitorAndCompleteCalculations(img, STARTS, ENDS, COL_START, COL_END)
