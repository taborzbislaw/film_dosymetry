...
class film_calibrationWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
    ...
    def saveCalibrationData(self) ->None:
        ...
        inputFname = self._parameterNode.inputVolume.GetStorageNode().GetFileName()
        saveDirName = os.path.dirname(inputFname)

        if len(pathCoefs) < 1:
            pathCoefs = saveDirName + '/kalibracja.json'

        if len(pathPlot) < 1:
            pathPlot = saveDirName + '/kalibracja.png'
