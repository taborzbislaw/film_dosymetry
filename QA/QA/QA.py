...
class QAWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
    ...
    def onApplyButton(self) -> None:
        ...
        ds = pydicom.dcmread(TPSDoseFileName,force=True)
        scaling = float(ds.DoseGridScaling)

        dose = slicer.util.arrayFromVolume(self._parameterNode.tpsDoseVolume)*scaling*100
        film = slicer.util.arrayFromVolume(self._parameterNode.filmVolume)[0]
        ...
        croppedVolume = slicer.mrmlScene.AddNewNodeByClass("vtkMRMLScalarVolumeNode", nodeName)
        slicer.util.updateVolumeFromArray(croppedVolume, cropped)    
        croppedVolume.SetOrigin(self._parameterNode.filmVolume.GetOrigin())
        croppedVolume.SetSpacing(self._parameterNode.filmVolume.GetSpacing())

        nodeName = 'registered TPS dose'
        registeredDose = slicer.mrmlScene.AddNewNodeByClass("vtkMRMLScalarVolumeNode", nodeName)
        slicer.util.updateVolumeFromArray(registeredDose, out)    
        registeredDose.SetOrigin(self._parameterNode.filmVolume.GetOrigin())
        registeredDose.SetSpacing(self._parameterNode.filmVolume.GetSpacing())

        workDir = os.path.dirname(__file__)
        fname = workDir + '/Resources/ref.nii.gz'
        saveImg = sitk.GetImageFromArray(out)
        saveImg.SetOrigin(self._parameterNode.filmVolume.GetOrigin())
        saveImg.SetSpacing(self._parameterNode.filmVolume.GetSpacing())
        sitk.WriteImage(saveImg,fname)

        fname = workDir + '/Resources/eval.nii.gz'
        saveImg = sitk.GetImageFromArray(cropped)
        saveImg.SetOrigin(self._parameterNode.filmVolume.GetOrigin())
        saveImg.SetSpacing(self._parameterNode.filmVolume.GetSpacing())
        sitk.WriteImage(saveImg,fname)
        ...
        slicerDir = os.getcwd()
        try:
            process = subprocess.Popen([slicerDir + "/bin/PythonSlicer", workDir + '/gamma.py',
                      str(self.ui.spinBoxPercentDose.value),str(self.ui.doubleSpinBoxDTA.value),
                      str(self.ui.spinBoxDoseThreshold.value)], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,preexec_fn=os.setpgrp)
        except:
            process = subprocess.Popen([slicerDir + "/bin/PythonSlicer", workDir + '/gamma.py',
                      str(self.ui.spinBoxPercentDose.value),str(self.ui.doubleSpinBoxDTA.value),
                      str(self.ui.spinBoxDoseThreshold.value)], stdout=subprocess.DEVNULL,    
                      stderr=subprocess.DEVNULL,creationflags=subprocess.HIGH_PRIORITY_CLASS&subprocess.DETACHED_PROCESS&subprocess.CREATE_NO_WINDOW)
        ...
        g = sitk.ReadImage(fname)
        gamma_image = sitk.GetArrayFromImage(g)

        nodeName = 'gamma cropped'
        gammaCroppedVolume = slicer.mrmlScene.AddNewNodeByClass("vtkMRMLScalarVolumeNode", nodeName)
        slicer.util.updateVolumeFromArray(gammaCroppedVolume, gamma_image)    
        gammaCroppedVolume.SetOrigin(self._parameterNode.filmVolume.GetOrigin())
        gammaCroppedVolume.SetSpacing(self._parameterNode.filmVolume.GetSpacing())

        GPR = (1.0 - len(np.where(gamma_image>=1.0)[0])/np.prod(gamma_image.shape))*100
        self.ui.labelGPR.text = f'Gamma index = {GPR:.2f}'
     
        CImg(cropped).display('film');
        CImg(out).display('registered TPS dose');
        CImg(gamma_image).display('gamma image');
